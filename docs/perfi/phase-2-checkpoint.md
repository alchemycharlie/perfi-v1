...
- **In-app walkthrough missing**: Phase 1 scope says "demo environment and short walkthrough". Phase 2 initially described demo data but no guided tour. **Resolved**: Added in-app walkthrough section (tooltip-based guided tour on first demo data load, 4 steps, dismissible, replayable from Settings).
- **Goals tier table ambiguity**: Pricing table said "Savings goals: Up to 2" but Phase 2 now distinguishes savings goals from financial goals. **Resolved**: Changed to "Goals (savings + financial): Up to 2" and updated upgrade prompt wording.
